...
export function DashboardPage() {
  const { kpiSummary, statusDistribution, isLoading, isError, error } = useAnalytics();
  const {
    tasks,
    users,
    usersMap,
    ...
    },
    {
      ...
      render: (task) => (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-semibold capitalize bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300">
          <span className={`w-2 h-2 rounded-full ${STATUS_COLORS[task.status] || 'bg-slate-400'}`} />
          {task.status.replace('-', ' ')}
        </span>
      ),
    },
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <Skeleton variant="rectangular" height="300px" className="lg:col-span-2" />
        ...
        <div className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xs lg:col-span-2 flex flex-col justify-between">
          <div>
            ...
            <div className="mt-3 flex items-center justify-between text-xs text-slate-500 dark:text-slate-400">
              <span>{sprintCompleted} of {sprintTasks.length} tasks completed</span>
              {activeSprint && (
                <span>
